...
const slideVariants = {
  enter: (dir: number) => ({ x: dir > 0 ? 380 : -380, opacity: 0 }),
  center: { x: 0, opacity: 1 },
  exit:  (dir: number) => ({ x: dir > 0 ? -380 : 380, opacity: 0 }),
};
            ...
            <AnimatePresence mode="wait" custom={dir}>
              <motion.div key={idx} custom={dir} variants={slideVariants}
                ...
                <div className="flex flex-col justify-center p-8 md:p-12">
                  ...
                  <p className="font-body text-sm leading-relaxed mb-7" style={{ color: "rgba(255,255,255,0.7)" }}>{c.desc}</p>

                  {/* Treatment tags */}
                  <div className="mb-7">
                    <div className="text-[10px] tracking-[0.15em] uppercase font-body mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>
                      Treatment Approach
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {c.tags.map((tag) => (
                        <span key={tag} className="px-3 py-1.5 rounded-full text-xs font-body"
                          style={{ background: `${c.color}18`, color: "rgba(255,255,255,0.8)", border: `1px solid ${c.color}30` }}>
                          {tag}
                        </span>
                      ))}
                    </div>
