        ...
        <div ref={headRef} className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[11px] tracking-[0.25em] uppercase font-body" style={{ color: "#E8001A" }}>What We Offer</span>
            </div>
            <h2 className="font-heading font-light text-white" style={{ fontSize: "clamp(36px,5vw,64px)" }}>
              Our <em style={{ fontStyle: "italic", color: "#E8001A" }}>Services</em>
            </h2>
          </div>
          <button
            ...
            style={{ background: "rgba(232,0,26,0.1)", border: "1px solid rgba(232,0,26,0.3)", color: "#E8001A" }}>
            Book Any Service
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {SERVICES.map((s, i) => (
            <TiltCard key={i} maxTilt={8}
              ...
              style={{ border: "1px solid rgba(232,0,26,0.08)" }}
              onClick={() => document.querySelector("#book")?.scrollIntoView({ behavior: "smooth" })}>
              <div className="p-7">
                {s.tag && (
                  <span className="inline-block px-2.5 py-1 rounded-full text-[10px] font-medium mb-4 font-body"
                    style={{ background: "rgba(232,0,26,0.15)", color: "#E8001A", border: "1px solid rgba(232,0,26,0.25)" }}>
                    {s.tag}
                  </span>
                )}
